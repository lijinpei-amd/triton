...
// Precise control-flow handling tests. ver01 uses AMD-side
// deduceMinCountOnDefChain (Utility.cpp:64) + deduceMinCountBetweeOps
// (Utility.cpp:21-52), which descends into scf.if (taking min across then/else),
// multiplies scf.for by its static trip count, and counts inner-region
// siblings before peeling. (Upstream-style minNumInterleavedCommitOps in
// WGMMAPipeline.cpp:67-68 conservatively skips into block ops' children -
// ver02/ver03 inherit that conservatism, see their tests for the differing
// CHECKs.)
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx950", "ttg.threads-per-warp" = 64 : i32} {
  ...
  // Sibling commit %3 contributes 1; deduceMinCountBetweeOps descends both
  // scf.if branches and adds min(1, 1) = 1. Total num = 2.
  // CHECK: ttg.async_wait %{{[^,]+}} {num = 2 : i32}
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx950", "ttg.threads-per-warp" = 64 : i32} {
  ...
  // The wait sits inside scf.if; the token is defined in the parent region.
  // deduceMinCountOnDefChain runs deduceMinCountBetweeOps(&block-front,
  // consumer, ...) before peeling each level, counting the two inner-region
  // commits (%b, %d) that precede the wait. num = 2.
  // CHECK: ttg.async_wait %{{[^,]+}} {num = 2 : i32}
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx950", "ttg.threads-per-warp" = 64 : i32} {
  ...
  // Sibling commit %3 contributes 1; deduceMinCountBetweeOps multiplies the
  // scf.for body's 1 commit per iter by its static trip count of 4, adding
  // 4. Total num = 5.
